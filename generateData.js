...
const formattedProducts = sampleProducts.map((p, i) => {
  let mainImage = '/assets/cat_decor.png';
  if (p.category === 'ALL HANDICRAFTS' && p.subcategory === 'Wooden Art Work') {
    mainImage = '/assets/cat_sculptures.png';
  } else if (p.subcategory === 'Wall Hangings') {
    mainImage = '/assets/cat_wall_art.png';
  } else if (p.category === 'GIFTS') {
  ...
  return {
    ...
    slug: slugify(p.name),
    name: p.name,
    category: p.category, // e.g. "ALL HANDICRAFTS"
    ...
    mainImage: mainImage,
    isBestSeller: i % 4 === 0 // Make every 4th product a best seller
  };
});

const content = \`export const localProducts = \${JSON.stringify(formattedProducts, null, 2)};\`;

fs.mkdirSync('./src/data', { recursive: true });
fs.writeFileSync('./src/data/products.js', content, 'utf-8');
console.log('Created src/data/products.js');
